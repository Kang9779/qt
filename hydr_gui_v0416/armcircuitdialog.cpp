...
void ArmCircuitDialog::startSave()
{
    ...
    SaveFile *saveFile=new SaveFile("C:\\Users\\ytc\\Desktop\\Software\\history\\"+fileName+"arm.txt");
    // 添加到ListWidget
...
void ArmCircuitDialog::displaySensor1(bool displayed)
{
...
void ArmCircuitDialog::displaySensor2(bool displayed)
{
    QImage *image=new QImage;
...
void ArmCircuitDialog::displaySensor3(bool displayed)
{
    QImage *image=new QImage;
...
void ArmCircuitDialog::displaySensor4(bool displayed)
{
    QImage *image=new QImage;
    image->load(":/transducer/rsc/pressure_right.png");
...
void ArmCircuitDialog::displaySensor5(bool displayed)
{
    QImage *image=new QImage;
    image->load(":/transducer/rsc/pressure_right.png");
...
void ArmCircuitDialog::addGraph(QCustomPlot *figure,QString title)
{
    figure->addGraph();
//    // 设置标题
    // 标题空白区域太大，采用legend方式
...
void ArmCircuitDialog::setPlot(QCustomPlot *figure)
{
    // 图形交互操作：缩放、平移等
...
void ArmCircuitDialog::setRealtimePlot(QCustomPlot *figureBoom)
{
    QSharedPointer<QCPAxisTickerTime> timeTicker(new QCPAxisTickerTime);
    timeTicker->setTimeFormat("%h:%m:%s");
...
// 布局子图设置
void ArmCircuitDialog::setCustomGraph(QCPGraph *graph)
{
    // 设置画笔
    QPen pen;
    pen.setColor(Qt::red);
