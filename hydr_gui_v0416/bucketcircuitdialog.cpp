...
#include "ui_bucketcircuitdialog.h"
...
BucketCircuitDialog::BucketCircuitDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::BucketCircuitDialog)
{
    ui->setupUi(this);
    // 关闭子窗口，返回主窗口
    connect(this,SIGNAL(rejected()),(MainWindow*)parentWidget(),SLOT(show()));
    // 加载油液回路动图
    movie = new QMovie(":/circuit/rsc/铲斗v2wukuang.gif");
    movie->setBackgroundColor(QColor(100,100,100,100));
    ui->labelImageBucket->setMovie(movie);
    ui->labelImageBucket->setScaledContents(true);
    // 循环播放动图
...
BucketCircuitDialog::~BucketCircuitDialog()
{
    delete ui;
}

void BucketCircuitDialog::loopMovie()
{
    movie->start();
}

void BucketCircuitDialog::switchPage(int index)
{
    switch(index)
    {
    case 1:
...
void BucketCircuitDialog::getRealtimeData(QString qstr)
{
    // QString类型转string类型
...
void BucketCircuitDialog::onRunStopChanged(int run)
{
    // 停止保存文件线程
    fileThread->quit();
    // 关闭定时器，禁止刷新曲线
...
void BucketCircuitDialog::updatePlot()
{
    if(sensor1->SensorSwitch->isChecked())
    {
...
void BucketCircuitDialog::updatePeriod(QString text)
{
    //调整定时器时间，改变图形刷新速率
    dateTimer->start(text.toInt());
}

void BucketCircuitDialog::startSave()
{
    // 创建并运行保存文件子线程
    QDateTime dt;
    QTime currTime;
    ...
    SaveFile *saveFile=new SaveFile("C:\\Users\\ytc\\Desktop\\Software\\history\\"+fileName+"bucket.txt");
    // 添加到ListWidget
...
void BucketCircuitDialog::openHistory()
{
...
void BucketCircuitDialog::evalHealth()
{
...
void BucketCircuitDialog::displaySensor1(bool displayed)
{
...
void BucketCircuitDialog::displaySensor2(bool displayed)
{
    QImage *image=new QImage;
...
void BucketCircuitDialog::displaySensor3(bool displayed)
{
    QImage *image=new QImage;
...
void BucketCircuitDialog::displaySensor4(bool displayed)
{
    QImage *image=new QImage;
    image->load(":/transducer/rsc/pressure_right.png");
...
void BucketCircuitDialog::displaySensor5(bool displayed)
{
    QImage *image=new QImage;
    image->load(":/transducer/rsc/pressure_right.png");
...
void BucketCircuitDialog::addGraph(QCustomPlot *figure,QString title)
{
    figure->addGraph();
//    // 设置标题
    // 标题空白区域太大，采用legend方式
...
void BucketCircuitDialog::setPlot(QCustomPlot *figure)
{
    // 图形交互操作：缩放、平移等
...
void BucketCircuitDialog::setRealtimePlot(QCustomPlot *figureBoom)
{
    QSharedPointer<QCPAxisTickerTime> timeTicker(new QCPAxisTickerTime);
    timeTicker->setTimeFormat("%h:%m:%s");
...
// 布局子图设置
void BucketCircuitDialog::setCustomGraph(QCPGraph *graph)
{
    // 设置画笔
    QPen pen;
    pen.setColor(Qt::red);
